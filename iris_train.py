...
def Spike_eval_converter(inputs,dt,time_,time_step,fr = 30,norm = 10):
    ...
    freq = freq_temp*np.repeat(np.expand_dims(inputs[time_],axis=0), time_step, axis= 0)
    spike_train = np.where(np.random.rand(time_step, 4) < freq*dt ,1 ,0)
    return spike_train
...
for time in range(len(X_train)):
    state_converted_train[time] = Spike_train_converter(inputs = X_train, dt = dt, time = time, time_step = time_step)

for time_ in range(len(X_test)):
    state_converted_eval[time_] = Spike_eval_converter(inputs=X_test,dt = dt,time_ = time_, time_step=time_step)
...
SNN_model = network.SNN_Net(num_time=args.num_time, l_tau=0.8,power=args.power,batch_size=args.batch,gpu=False) #l_tau : more complicated spikign neuron
